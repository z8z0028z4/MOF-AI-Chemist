...
def build_pdf_candidate_urls(pmcid: str, pdf_url: str = "") -> List[str]:
    """Return the preferred Europe PMC render URL for a PubMed Central article."""
    normalized_pmcid = pmcid.strip()
    if not normalized_pmcid:
        return []
    return [f"https://europepmc.org/articles/{normalized_pmcid}?pdf=render"]


def is_pdf_response(response: requests.Response) -> bool:
    """Accept real PDF bytes even when a provider sends a generic content type."""
    content_type = response.headers.get("Content-Type", "").lower()
    return response.content.startswith(b"%PDF") or "pdf" in content_type
...
def download_and_store(record: Dict, folder: str) -> str:
    ...
    for candidate_url in build_pdf_candidate_urls(pmcid, pdf_url):
        try:
            # 發送HTTP請求下載PDF
            r = requests.get(
                candidate_url,
            ...
            print(f"⚠️ PDF響應異常：{candidate_url} status={r.status_code} content-type={content_type}")

        except Exception as e:
            print(f"❌ 下載錯誤：{candidate_url} -> {e}")
