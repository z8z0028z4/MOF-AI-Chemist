...
def generate_structured_proposal(chunks: List, question: str, model: Optional[str] = None, mof_linker_mode: str = "auto") -> Dict[str, Any]:
    """
    生成結構化研究提案
    ...
    # 調用結構化LLM
    proposal_data = call_llm_structured_proposal(system_prompt, user_prompt, model=model, mof_linker_mode=mof_linker_mode)
...
def generate_iterative_structured_proposal(
    ...
    # 調用結構化LLM
    proposal_data = call_llm_structured_proposal(system_prompt, user_prompt, model=model)
...
def generate_structured_experimental_detail(chunks: List, proposal: str, user_prompt: str = "", model: Optional[str] = None) -> Dict[str, Any]:
    """
    生成結構化實驗細節的便捷函數
    ...
    # 調用結構化LLM
    experimental_data = call_structured_llm(full_prompt, current_schema, model=model)
...
def generate_structured_revision_proposal(
    ...
    """
    return call_llm_structured_revision_proposal(question, new_chunks, old_chunks, proposal, model=model, mof_linker_mode=mof_linker_mode)
...
def generate_structured_revision_experimental_detail(
    ...
    """
    return call_llm_structured_revision_experimental_detail(question, new_chunks, old_chunks, proposal, original_experimental_detail, model=model)
